from RandomForest import getPathRow
...
def createClassifier(inraster,inshp,field:str="Id",treenum:int=100):
    ...
    spatial2.ImportFromWkt(rasterspatial.GetProjectionRef())  # 使用栅格的投影信息
    shpspatial=ogr.Open(inshp)
    layer=shpspatial.GetLayer(0)
    spatial1=layer.GetSpatialRef()   # 矢量投影信息
   
    ct=osr.CreateCoordinateTransformation(spatial1,spatial2)
    oFeature = layer.GetNextFeature()
    # 下面开始遍历图层中的要素
    geom=oFeature.GetGeometryRef()   # 查看几何类型
    ...
    while oFeature is not None:
        ...
        for point in points:
            pC=ct.TransformPoint(point.X,point.Y,0)   # 变换坐标系
