...
def RandomForestClassification(inRaster, model, outfile, blockSize=0): 
    ...
    clf = model   #tbrf.createClassifier(TrainRaster,TrainShp)
    Z = list()
    fixX = list()
    if blockSize == 0:
        p,a = memory_usage()
...
if __name__ == '__main__':
    ...
    outfile = r"D:\vscode\pythonscript\image_deal\image-classified\随机森林分类\Python+Scikit-Learn+RandomForest\data\result4.tif"
    trainX, trainY = tbrf.createDataSet(rasterPath, shpPath)
    model = tbrf.trainModel(trainX, trainY)
    RandomForestClassification(rasterPath, model, outfile)
